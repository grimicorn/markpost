...
**Request body.** The delivery body must be a JSON object (send `Content-Type: application/json`). GitHub's default `application/x-www-form-urlencoded` content type is also accepted, since GitHub URL-encodes the JSON under a `payload` form field and computes the signature over that raw body. Anything else — a plain-text body, a generic form-encoded body, a JSON scalar/array, or an empty body — is rejected with `400 Bad Request` rather than silently ingesting a blank "Untitled" record, so a misconfigured integration fails loudly in the sender's delivery log instead of quietly producing empty records. Field mapping (see below) still runs on the object, so keys can be remapped to `title`/`body`, but the top-level payload itself must be an object.
