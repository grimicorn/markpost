<template>
  <div class="scroll" style="height: 100vh; overflow-y: auto; background: var(--bg)">
    ...
    >
      ...
      >
        <div>
          ...
          <h1 class="display" style="margin-top: 22px">
            Every webhook<br />and email, as<br /><span class="outline">Markdown.</span>
          </h1>
          <p class="lead" style="margin-top: 24px; max-width: 440px">
            markpost catches inbound webhooks and email, converts them to clean Markdown with
            frontmatter, and the CLI syncs them straight into your Obsidian vault — on your
            machine.
          </p>
          <div class="row gap-3 wrap" style="margin-top: 30px">
            <AppBtn variant="accent" size="lg" icon-r="arrowR" href="/pricing">
              start free trial
            </AppBtn>
            <AppBtn size="lg" icon="book" href="/docs">read the docs</AppBtn>
          </div>
          <div class="row gap-4 wrap mono" style="margin-top: 26px; font-size: 12px; color: var(--ink-3)">
            <span class="row gap-2">
              <AppIcon name="check" :size="14" :style="{ color: 'var(--ok)' }" />no card to start
            </span>
            <span class="row gap-2">
              <AppIcon name="check" :size="14" :style="{ color: 'var(--ok)' }" />14-day Pro trial
            </span>
            <span class="row gap-2">
              <AppIcon name="check" :size="14" :style="{ color: 'var(--ok)' }" />your files, your disk
            </span>
          </div>
        </div>
        ...
        <div class="term">
          ...
          <div class="term-body">
            <div><span class="pr">$</span> markpost sync <span class="c-dim">--watch</span></div>
            <div class="c-dim" style="margin-top: 6px">→ connected to ingest.markpost.io</div>
            <div class="c-dim">→ watching for inbound records…</div>
            <div style="margin-top: 10px">
              <span class="c-acc">webhook</span> github:push <span class="c-dim">→</span>
              <span class="c-file">99-incoming/2026-06-14-deploy.md</span>
              <span class="c-ok">✓</span>
            </div>
            <div>
              <span class="c-acc">email</span>&nbsp;&nbsp; clip@markpost <span class="c-dim">→</span>
              <span class="c-file">99-incoming/swift-concurrency.md</span>
              <span class="c-ok">✓</span>
            </div>
            <div>
              <span class="c-acc">webhook</span> stripe:invoice <span class="c-dim">→</span>
            ...
            <div style="margin-top: 4px">
              <span class="pr">$</span>
              <span style="border-left: 7px solid var(--accent); margin-left: 2px">&nbsp;</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- sources strip -->
      <div class="row gap-6 wrap" style="margin-top: 48px; position: relative">
        ...
        <div class="row gap-2 wrap">
          ...
          <AppChip v-for="preset in presets" :key="preset">{{ preset }}</AppChip>
        </div>
      </div>
    </AppReveal>
    ...
    <AppReveal
      as="section"
      :style="{ padding: '56px 40px 16px', maxWidth: '1180px', margin: '0 auto' }"
    >
      <div class="row between wrap" style="align-items: flex-end; margin-bottom: 36px">
        <div>
          <AppEyebrow>how it works</AppEyebrow>
          <h2 class="h1" style="margin-top: 14px">Inbox to vault in three steps.</h2>
        </div>
      ...
      <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px">
        <div v-for="step in steps" :key="step.n" class="card card-pad">
          <div class="row between">
            ...
            <span class="mono" style="font-size: 13px; color: var(--ink-3)">{{ step.n }}</span>
          </div>
          <h3 class="h3" style="margin-top: 18px">{{ step.t }}</h3>
          <p class="muted" style="margin-top: 8px; font-size: 14.5px; line-height: 1.55">
            {{ step.d }}
          </p>
        </div>
      </div>
    ...
    <AppReveal
      as="section"
      :style="{ padding: '44px 40px 64px', maxWidth: '1180px', margin: '0 auto' }"
    >
      <div
        class="panel"
        style="overflow: hidden; display: grid; grid-template-columns: 1fr 1.1fr"
      >
        <div style="padding: 40px">
          <AppEyebrow>the output</AppEyebrow>
          <h2 class="h2" style="margin-top: 16px">Plain Markdown. Real frontmatter.</h2>
          <p class="muted" style="margin-top: 12px; font-size: 15px; line-height: 1.6">
            No proprietary format, no lock-in. Every record is a portable
            <code class="mono" style="font-size: 13px; background: var(--bg-2); padding: 1px 5px; border-radius: 4px">.md</code>
            file your editor already understands — fully templated to match your vault's
            conventions.
          </p>
        ...
        <div
          class="code"
          style="border-radius: 0; border-top: 0; border-right: 0; border-bottom: 0"
        >
          <div class="code-head">
...
Commit <span class="s">a1f9c20</span> shipped to **prod**
by @dan in 47s. All checks green.

> synced by markpost</div>
        </div>
      </div>
    </AppReveal>
    ...
    >
      ...
      >
        ...
        <div style="position: relative">
          ...
          <p class="lead" style="margin-top: 12px; max-width: 460px; margin-inline: auto">
          ...
          <div class="row gap-3" style="justify-content: center; margin-top: 26px">
            ...
            <AppBtn size="lg" icon="terminal" href="/docs">view CLI reference</AppBtn>
          </div>
        </div>
      </div>
    </AppReveal>
