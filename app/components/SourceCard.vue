      ...
      <div class="row gap-3">
        ...
        <div class="col" style="gap: 3px">
          <span class="row gap-2" style="align-items: center">
            <span style="font-weight: 600; font-size: 15px">{{ source.name }}</span>
            <AppChip v-if="source.via" style="font-size: 10px; padding: 1px 6px">
              via {{ source.via }}
            </AppChip>
          </span>
          <span class="mono faint" style="font-size: 11.5px">{{ source.sub }}</span>
        </div>
      </div>
    ...
    <div class="code" style="margin-top: 16px">
      ...
      <div class="code-body mono" style="font-size: 12.5px; word-break: break-all">
        <template v-if="source.endpointHighlight">
          {{ endpointBefore }}<span style="color: var(--accent-700)">{{ source.endpointHighlight }}</span>{{ endpointAfter }}
        </template>
        <template v-else>{{ source.endpoint }}</template>
      </div>
    </div>

    <div class="row gap-6 wrap mono" style="font-size: 11.5px; color: var(--ink-3); margin-top: 14px">
...
const endpointBefore = computed(() => {
  ...
  return index >= 0 ? props.source.endpoint.slice(0, index) : props.source.endpoint;
});
...
const endpointAfter = computed(() => {
  ...
  const index = props.source.endpoint.indexOf(props.source.endpointHighlight);
  return index >= 0 ? props.source.endpoint.slice(index + props.source.endpointHighlight.length) : "";
});
</script>
